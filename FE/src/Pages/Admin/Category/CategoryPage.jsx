...
import { fetchAllCategory, fetchDeleteCategory } from "../../../features/category/categoryThunk";

function CategoryPage() {
  const { data, filters, pagination } = useCategory();
  ...
  const handleChangePage = (e, p) => {
    dispatch(categoryActions.setFilters({ ...filters, page: p }));
  };

  const handleDelete = (id) => {
    const confirm = window.confirm("Are you sure delete category with id = " + id);

    if (confirm) {
      dispatch(fetchDeleteCategory(id)).then((t) => {
              ...
              data.map((row) => (
                <TableRow key={row.id} sx={{ "&:last-child td, &:last-child th": { border: 0 } }}>
                  ...
                  <TableCell align="right">
                    <IconButton
                      component={Link}
                      to={`/admin/category/update/${row.id}`}
                      aria-label="see"
                      color="primary"
                    >
                      <EditIcon />
